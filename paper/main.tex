...
range, $N\!=\!5$]. Day~0 is the state after the first 6\,h of ODE
...
onset-shift proxy rather than re-simulating $N\!=\!5$ per cell --- is
...
on influenza's hour-to-day timescales. With $N\!=\!5$, runtime bounds
are sample min--max, not stable $[2.5, 97.5]$ percentile estimates
($N\!\geq\!20$); production needs a larger ensemble (wall-clock per
tick is $\max(T_i)$ thanks to concurrent subprocesses). The
...
model. Over 14 days ($N\!=\!5$, 56 GSimT checkpoints), ISSL records
reproduce viral kinetics (peak $V=9.0{\times}10^{6}$ at day~2.25;
$<\!0.1\%$ by day~13.75) and spatial immune recruitment (median
$n_{\text{immune}}\!=\!6$ at day~1, 50 at day~13). Immune temporal lag
