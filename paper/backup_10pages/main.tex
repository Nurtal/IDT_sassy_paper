...
$N\!=\!5$ parallel CC3D instances (concurrent subprocesses, distinct
MersenneTwister seeds); runtime uncertainty bounds --- reported as
\emph{ensemble range across $N\!=\!5$ stochastic CC3D replicates}
(sample min--max, see \S\ref{sec:valid-setup}) --- originate in the
...
$<\!0.1\%$ of peak by day~13.75) and spatial immune recruitment (median
$n_{\text{immune}}\!=\!6$ [ens.\,range: 4--7] at day~1, growing to 50
[ens.\,range: 40--53] at day~13), with immune temporal lag and
...
in the stochastic ABM ensemble ($N\!=\!5$ parallel CC3D instances; with
$N\!=\!5$, reported bounds correspond to sample min--max, not stable
percentile estimates) and propagate through the deterministic ODE via
triple integration at each checkpoint (\S\ref{sec:valid-setup}).
...
\item \textbf{Stochastic--deterministic reconciliation.} Signals from a
      ...
      (\S\ref{sec:valid-setup}; with $N\!=\!5$, these correspond to
      sample min--max rather than stable percentile estimates); for
...
structured in six sections (Table~\ref{tab:issl}). JSON-LD enables both
human readability and machine-parsable semantic annotation via linked
data context, allowing orchestrator components to parse ISSL records
without prior knowledge of the model's internal variable naming
conventions.

\begin{table}[t]
\centering
\caption{ISSL section inventory. The \texttt{export\_signals} field
always carries the biologically scaled value, ensuring downstream
models receive correct quantities regardless of source formalism.}
\label{tab:issl}
\renewcommand{\arraystretch}{1.0}
\begin{tabularx}{\columnwidth}{@{}lX@{}}
\toprule
\textbf{ISSL section} & \textbf{Content / parsability note}\\
\midrule
\texttt{envelope}              & Model ID, version, GSimT timestamp, schema URI, formalism tag. Orchestrator validates \texttt{schema\_uri}; reads \texttt{formalism} before processing outputs.\\[1pt]
\texttt{continuous\_state}     & Running entity populations: count (scaled), unit, \texttt{ci\_95} (ensemble bounds). \texttt{label} is model-local; orchestrator maps via signal\_id routing.\\[1pt]
\texttt{discrete\_events}      & Punctual events: cell death, cytokine peaks. \texttt{event\_type} from controlled vocabulary; ABM events include \texttt{n\_realisations}.\\[1pt]
\texttt{export\_signals}       & Inter-compartmental signals available for routing. \texttt{signal\_id} is the routing key; \texttt{value} in declared \texttt{unit}.\\[1pt]
\texttt{internal\_parameters}  & Kinetic parameter values; \texttt{provenance} is DOI or database URI linking to the calibration source.\\[1pt]
\texttt{watchdog}              & Model health: status, \texttt{divergence\_score}. Values $>\!0.15$ $\Rightarrow$ \texttt{PAUSE}.\\
\bottomrule
\end{tabularx}
\end{table}
...
\medskip
\textit{Box 2 --- ISSL excerpt, Sego 2020 CC3D ABM adapter (day 1
checkpoint). Values are from a single illustrative CC3D instance and
may differ from the ensemble median ($N\!=\!5$) reported in
\S\ref{sec:valid-bio}.}
\begin{lstlisting}[language=json]
{
 "envelope": {
   ...
   "model_version":
      "github:covid-tissue-models@5b7e42c+OISA_bridge",
   "sim_time_s": 86400,
   "formalism": "ABM",
   "agent_count": 7,
   "grid": "90x90x2"
 ...
 "export_signals":[
   {"signal_id":"sego2020.immune_cell_count",
    "value":7,"unit":"cells","ci_95":[5,8],
    "transfer_lag_s":null},
   {"signal_id":"sego2020.recruitment_cytokine",
    "value":700.0,"unit":"AU","ci_95":[500.0,800.0],
    "transfer_lag_s":null}
 ],
 "watchdog":{"status":"OK","divergence_score":0.0}
}
...
the composition (Table~\ref{tab:config}). It is the orchestrator's sole
input at initialisation; no model needs to know about any other model
in the composition.
...
\begin{table}[t]
\centering
\caption{Configuration graph schema. The declarative separation of
model identity, wiring, and temporal parameters implements the CURE
extensibility requirement: any model node can be substituted without
modifying the graph structure.}
\label{tab:config}
\renewcommand{\arraystretch}{1.0}
\begin{tabularx}{\columnwidth}{@{}lX@{}}
\toprule
\textbf{Element} & \textbf{Description / example}\\
\midrule
\texttt{models[]}            & \{\,id, formalism: ODE$\vert$ABM, executable, issl\_port, delta\_t\_s\,\}\\[1pt]
\texttt{edges[]}             & \{\,source\_model, signal\_id, target\_model, lag: "constant:N" $\vert$ "model:ID"\,\}\\[1pt]
\texttt{transfer\_models[]}  & \{\,id, formalism, executable, input\_signal, output\_signal, lag\_output\_field\,\}\\[1pt]
\texttt{global\_clock}       & \{\,start\_s, end\_s, checkpoint\_interval\_s\,\}\\[1pt]
\texttt{calibration}         & \{\,data\_source\_uri, patient\_id, recalibration\_trigger, biomarker\_map[\,]\,\}\\[1pt]
\texttt{wildcard\_namespace} & \{\,prefix, registry\_uri: null\,\} --- allows non-OBO entities (e.g.\ novel murine cytokines or patient-specific biomarkers).\\
\bottomrule
\end{tabularx}
\end{table}
...
The orchestrator is a nine-component server process
(Table~\ref{tab:orchestrator}). It reads the configuration graph at
startup, establishes connections to all model processes, and manages
the simulation run.

\begin{table}[t]
\centering
\caption{Orchestrator component inventory. Status column: \checkmark{}
implemented and covered by the published test suite (67 tests); $\circ$
interface specified, full implementation deferred. Components marked
\checkmark/$\circ$\textsuperscript{$\dagger$} have core functionality
implemented and tested (ISSL parsing, parameter bounds checking,
checkpoint snapshots, divergence threshold) while specific
sub-capabilities --- OOD detection, \texttt{ROLLBACK} recovery,
PROV-O URIs, conservation violation auto-rollback --- are specified
but not yet in the published test suite.}
\label{tab:orchestrator}
\renewcommand{\arraystretch}{1.0}
\begin{tabularx}{\columnwidth}{@{}p{3.1cm}Xc@{}}
\toprule
\textbf{Component} & \textbf{Responsibility / note} & \textbf{Status}\\
\midrule
1. ISSL ingestion          & JSON-LD parser; SI unit normaliser; OOD detector (Mahalanobis)        & \checkmark/$\circ$\textsuperscript{$\dagger$}\\
2. Temporal scheduler      & Priority queue by \texttt{next\_step\_due}; heterogeneous $\Delta t$ & \checkmark\\
3. Causal resolver         & Topological sort; cycle detection; feedback edges get one-tick delay & \checkmark\\
4. Constraint engine       & Mass conservation; parameter bounds; raises \texttt{CONSTRAINT\_VIOLATION} & \checkmark/$\circ$\textsuperscript{$\dagger$}\\
5. State registry          & Immutable versioned snapshots per GSimT checkpoint; provenance graph & \checkmark/$\circ$\textsuperscript{$\dagger$}\\
6. Transfer dispatcher     & Invokes transfer model on-demand; maintains \texttt{SignalQueue}     & \checkmark\\
7. Calibration bridge      & EHR ingest; triggers recalibration; broadcasts updated priors        & $\circ$\\
8. Output aggregator       & Merges GIS into checkpoint log; uncertainty propagation              & \checkmark\\
9. Watchdog monitor        & Poll model health; pause / resume / rollback                         & \checkmark/$\circ$\textsuperscript{$\dagger$}\\
\bottomrule
\end{tabularx}
\end{table}
...
(Table~\ref{tab:config}, \texttt{calibration} element), but its
implementation in the reference orchestrator is limited to schema
validation of the \texttt{calibration} configuration block; full EHR
ingestion and dynamic recalibration are deferred to clinical
deployment.
...
\textbf{Adapter metrics.} The total modification to either published
model is zero lines (Table~\ref{tab:adapter}). The Miao 2010 adapter
wraps libroadrunner ($\sim$60 lines). The Sego 2020 CC3D adapter
manages the subprocess lifecycle and IPC protocol ($\sim$180 lines),
complemented by the \texttt{OISABridgeSteppable} internal to CC3D
($\sim$60 lines); combined $\sim$240 adapter lines with zero Sego 2020
source modifications.

\begin{table}[t]
\centering
\caption{Lines of code modified vs.\ added per model. The ``zero
lines modified'' property directly demonstrates OISA's non-invasive
...
\begin{tabular}{@{}lllcc@{}}
\toprule
\textbf{Model}     & \textbf{Format}   & \textbf{Source}   & \textbf{Mod.} & \textbf{Adapter}\\
\midrule
Miao 2010 ODE      & SBML XML          & BIOMD0000000546   & \textbf{0}    & $\sim$60\\
Sego 2020 CC3D ABM & CC3D (Py/XML)     & GitHub @5b7e42c   & \textbf{0}    & $\sim$240\\
\bottomrule
\end{tabular}
\end{table}

\textbf{Runtime UQ.} The ABM component runs as a rolling ensemble of
$N\!=\!5$ parallel CC3D instances (distinct MersenneTwister seeds,
...
\texttt{n\_immune} across instances and computes the median and
empirical bounds. \emph{Statistical note:} with $N\!=\!5$ instances,
the \texttt{ci\_95} field reports the sample minimum and maximum
(empirical $p_0$ and $p_{100}$), not stable estimates of the
2.5\textsuperscript{th} and 97.5\textsuperscript{th} percentiles, which
would require $N\!\geq\!20$. We retain the schema field name
\texttt{ci\_95} for format consistency but label all reported bounds as
``ensemble range [$N\!=\!5$]'' throughout. The ODE adapter receives the
...
\midrule
$V(0)$ viral inoculum                  & 1{,}000 copies/mL              & 1{,}000                 & \checkmark\\
Ep(0) uninfected cells                 & 580{,}000                      & 580{,}000               & \checkmark\\
...
ran for 14 days (56 GSimT checkpoints at 6\,h intervals) with $N\!=\!5$
parallel CC3D instances. Runtime ensemble bounds are computed at each
tick by the ensemble adapter and propagated through the ODE. Values
below are median [ensemble range, $N\!=\!5$] from runtime ISSL records
(Table~\ref{tab:trajectory}).

\begin{table}[t]
\centering
\caption{14-day coupled trajectory. Median [ensemble range, $N\!=\!5$].
Day 0 corresponds to the state after the first GSimT tick (6\,h of
ODE integration from $V_0\!=\!1{,}000$ copies/mL).}
...
Day 1  & $3.29{\times}10^{5}$ & \textbf{6 [4--7]}\\
Day 2  & $8.88{\times}10^{6}$ & \textbf{11 [10--12]}\\
Day 3  & $6.27{\times}10^{6}$ & 15 [12--19]\\
Day 4  & $3.36{\times}10^{6}$ & 18 [14--24]\\
Day 5  & $1.78{\times}10^{6}$ & 23 [20--27]\\
Day 6  & $9.33{\times}10^{5}$ & 28 [26--32]\\
Day 7  & $4.85{\times}10^{5}$ & 30 [29--32]\\
Day 8  & $2.51{\times}10^{5}$ & 33 [32--39]\\
Day 9  & $1.29{\times}10^{5}$ & 39 [36--41]\\
Day 10 & $6.58{\times}10^{4}$ & 40 [37--43]\\
Day 11 & $3.34{\times}10^{4}$ & 44 [40--47]\\
Day 12 & $1.69{\times}10^{4}$ & 47 [44--51]\\
Day 13 & $8.43{\times}10^{3}$ & 50 [45--53]\\
\bottomrule
\end{tabular}
\end{table}

Median peak $V=9.03{\times}10^{6}$\,copies/mL at day 2.25; median $V$ at
day 13.75 = $4.97{\times}10^{3}$\,copies/mL $=$ 0.06\% of peak. All 5
ensemble instances confirmed clearance ($V(\text{day 14}) < 0.1\%$ of
peak). The ensemble bounds on \texttt{n\_immune} are narrow on days
...
\texttt{n\_immune} across $N\!=\!5$ replicates with IQR shading and
...
6 (day 1) to 50 (day 13), $T_{E_T}\!=\!n_{\text{immune}}\!\times\!100$
grows from 600 to 5{,}000\,CTL/mL in the Miao 2010 SBML, progressively
...
\subsubsection{Parameter Sensitivity (ODE-side, coupling parameters only)}
\label{sec:valid-sens}

This is an \emph{approximate single-model} sensitivity sweep of the two
coupling parameters only; the full ABM ensemble is not re-simulated
for each grid cell. Sensitivity of the \emph{model-internal} parameters
of Miao 2010 ($\beta_a$, $c_V$, $\delta_{Es}$, \dots) and Sego 2020
(\texttt{ir\_prob\_scaling\_factor}, chemotaxis coefficients,
\texttt{max\_ck\_secrete\_infect}, \dots) is out of scope for this
framework paper, since those parameters have already been calibrated
against published experimental data by the original authors; a full
coupled global sensitivity analysis (e.g.\ Sobol indices over both
model-internal and coupling parameters) is deferred to future work.

The two empirically set coupling parameters --- $\kappa =
3.5{\times}10^{-7}$\,AU$\cdot$mL/copies (ODE~$\to$~ABM) and the scaling
factor \texttt{\_N\_IMMUNE\_TO\_CTL\_PER\_ML} $=$ 100\,CTL/mL per CC3D
agent (ABM~$\to$~ODE) --- were varied over $\pm$1 order of magnitude in
a $3{\times}3$ grid (Table~\ref{tab:sensitivity}). For each $\kappa$,
the \texttt{n\_immune} trajectory was \emph{approximated} by shifting
the reference ensemble median by the qualitatively observed
$\pm 1$--$2$ day onset shift, rather than re-running the full
$N\!=\!5$ CC3D ensemble; the ODE was then integrated for 14 days with
the shifted trajectory. An isolated ODE baseline ($T_{E_T}=0$
throughout) is included for comparison (peak
$V\!=\!9.09{\times}10^{6}$, $V(\text{day 14})\!=\!1.02{\times}10^{4}$).

\begin{table}[t]
\centering
\caption{Sensitivity of coupled viral trajectory to $\kappa$ and
scaling factor. Bold row: reference parameter values used in
\S\ref{sec:valid-bio}.}
...
\begin{tabular}{ccccc}
\toprule
\textbf{$\kappa$} & \textbf{scaling} & \textbf{peak $V$} & \textbf{peak day} & \textbf{$V_{14}/V_{\text{peak}}$}\\
\midrule
$3.5{\times}10^{-8}$ &  10  & $9.09{\times}10^{6}$ & 2.50 & 0.108\%\\
$3.5{\times}10^{-8}$ & 100  & $9.09{\times}10^{6}$ & 2.50 & 0.077\%\\
$3.5{\times}10^{-8}$ & 1000 & $9.09{\times}10^{6}$ & 2.50 & 0.003\%\\
$\mathbf{3.5{\times}10^{-7}}$ &  10  & $9.08{\times}10^{6}$ & 2.50 & 0.105\%\\
$\mathbf{3.5{\times}10^{-7}}$ & \textbf{100}  & $\mathbf{9.01{\times}10^{6}}$ & \textbf{2.50} & \textbf{0.055\%}\\
$\mathbf{3.5{\times}10^{-7}}$ & 1000 & $8.39{\times}10^{6}$ & 2.25 & $<$0.001\%\\
$3.5{\times}10^{-6}$ &  10  & $9.08{\times}10^{6}$ & 2.50 & 0.102\%\\
$3.5{\times}10^{-6}$ & 100  & $8.94{\times}10^{6}$ & 2.50 & 0.044\%\\
$3.5{\times}10^{-6}$ & 1000 & $7.84{\times}10^{6}$ & 2.25 & $<$0.001\%\\
\bottomrule
\end{tabular}
\end{table}

The sensitivity analysis reveals two findings. First, the scaling
factor has the dominant effect: increasing it by $10{\times}$
accelerates viral clearance by 2--3 orders of magnitude at day 14. This
is expected because the scaling factor directly multiplies $T_{E_T}$
in the CTL killing term. Second, $\kappa$ has a secondary effect
mediated through its influence on immune recruitment timing. Across
all 9 combinations, peak $V$ remains within
$[7.84{\times}10^{6},\,9.09{\times}10^{6}]$\,copies/mL --- the same
order of magnitude as the published Miao 2010 fit --- and viral
clearance ($V_{14}\!<\!1\%$ of peak) is achieved in all cases. The
qualitative trajectory shape is preserved across the full parameter
grid, confirming that the coupled dynamics are robust to the coupling
parameter choices within $\pm 1$ order of magnitude.
...
Three points deserve emphasis. First, OISA implements CURE credibility
at the \emph{composition} level: a composition can violate biological
plausibility even when each constituent model is individually valid ---
e.g., a mis-specified tissue-to-systemic scaling factor silently
miscalibrates CTL killing. The explicit
\texttt{\_N\_IMMUNE\_TO\_CTL\_PER\_ML = 100} parameter makes this
decision auditable in the ISSL record. Second, \texttt{model\_version}
ties every checkpoint to a citable, immutable artefact. Third, zero
internal biological modifications across two published models
operationalises CURE extensibility at the multi-model scale.

\subsection{Limitations}

\textbf{Validation scope.} Validation is on a single organism (murine)
and disease (influenza A); the coupled model has not been validated
against in-vivo longitudinal tissue-level data, which are not
published in directly comparable form.

\textbf{Epithelial depletion.} In Miao 2010, the high infection rate
($\beta_a\!=\!10^{-6}$\,mL$\cdot$copies$^{-1}\cdot$day$^{-1}$) drives
near-complete epithelial depletion by day~2--3, after which clearance
is driven by $\delta_{Es}$ and CTL killing. This is an intrinsic
property of Miao 2010 and does not affect orchestration validation.

\textbf{Feedback loops.} The composition graph contains an ODE $\to$
ABM $\to$ ODE cycle. The 6\,h back-edge delay is necessary for causal
correctness and introduces a bounded lag that is negligible on
influenza's hour-to-day kinetic timescales.

\textbf{ABM stochasticity and ensemble size.} With $N\!=\!5$, runtime
bounds are sample min--max, not stable $[2.5, 97.5]$ percentile
estimates (which require $N\!\geq\!20$). Production deployment requires
a larger ensemble; wall-clock per tick is $\max(T_i)$ thanks to
concurrent subprocesses.

\textbf{Scaling factor and spatial scale mismatch.} The
\texttt{\_N\_IMMUNE\_TO\_CTL\_PER\_ML\,$=\,$100} conversion is an
approximation; more fundamentally, the Miao 2010 ODE
($V_{\text{total}}\!\approx\!1$\,mL) and the Sego 2020 tissue patch
($\approx\!10^{-3}$\,mL) differ by $\sim$3~OOM in volume. We omit
volume normalisation because (i)~the present target is orchestration
correctness, not biological prediction, and (ii)~the empirically tuned
$\kappa$ absorbs the mismatch implicitly. Predictive deployment must
introduce explicit $V_{\text{tissue}}\!=\!V_{\text{systemic}}\times
(V_{\text{patch}}/V_{\text{total}})$ normalisation and recalibrate
$\kappa$ and the scaling factor against paired tissue + systemic data.

\textbf{IPC performance.} The file-based IPC adds $\approx$50\,ms per
tick ($<$1\,s total over 14 days), negligible vs.\ CC3D time.
Sub-6-hour inter-model communication would require socket or
shared-memory transport (2--3 OOM latency reduction).
...
BIOMD0000000546) and the full Sego 2020 CC3D ABM
(12 steppables, $90{\times}90{\times}2$ grid, @5b7e42c) with zero
changes to either model. Over 14 days ($N\!=\!5$ rolling ensemble, 56
GSimT checkpoints), ISSL records reproduce Miao 2010 viral kinetics
(peak $V=9.0{\times}10^{6}$ at day~2.25; $<\!0.1\%$ of peak by
day~13.75) and spatial immune recruitment (median
$n_{\text{immune}}\!=\!6$ [4--7] at day~1; 50 [40--53] at day~13).
Immune temporal lag and CTL-mediated clearance acceleration emerge as
bidirectional-coupling properties that neither model alone produces.
Formalism-agnostic composition --- including full spatial ABMs as
independent processes with runtime UQ --- is achievable through
adapter-only integration with no model rewrites.
...
orchestrator, adapter code ($\sim$300 lines), configuration graph,
14-day run data (280 ISSL files, \texttt{results/issl\_14d/}),
sensitivity grid, figure scripts, and the full test suite (67 adapter
tests + 51 paper--data consistency tests) are available at
[\emph{repository URL to be added prior to submission}]. Miao 2010 SBML
is from BioModels unmodified; Sego 2020 is from
\texttt{covid-tissue-models}@\texttt{5b7e42c} under CompuCell3D 4.8.0.
